...
namespace TestUtils {
    ...
    #define ASSERT_(Assumption, CustomErrMsg, DoCAssert, DoLog, DoThrow)\
    ...
    if (const volatile auto assert__result__local__temp__ = !(Assumption)) {\
      if (DoCAssert) assert(! #Assumption); /*'assert' used ONLY in the debug mode*/\
  ...
  #ifndef ASSERT__
    // Uses auto. generated err. msg. & NO break the program on error (default mode)
    #define ASSERT__(Assumption) ASSERT_(Assumption, "", false, true, false)
  #else
